...
def text_to_ids(source_text, target_text, source_vocab_to_int, target_vocab_to_int):
    ...
    for line in target_text.split('\n'):
        ...
        tmp += [target_vocab_to_int['<EOS>']]
        target_letter_ids += [tmp]
...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob, 
    ...
    enc_cell = tf.contrib.rnn.MultiRNNCell([tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.LSTMCell(rnn_size, initializer=tf.random_uniform_initializer(-0.1, 0.1, seed=2)),input_keep_prob=keep_prob) for _ in range(num_layers)])
...
def decoding_layer(dec_input, encoder_state,
    ...
    training_decoder = decoding_layer_train(encoder_state, dec_cell, dec_embed_input, 
                         target_sequence_length, max_target_sequence_length, 
                         output_layer, keep_prob)
...
def seq2seq_model(input_data, target_data, keep_prob, batch_size,
    ...
    training_decoder_output, inference_decoder_output = decoding_layer(dec_input,enc_state,target_sequence_length,max_target_sentence_length,rnn_size,num_layers,target_vocab_to_int, target_vocab_size,
